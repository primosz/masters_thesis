...
def main():
    ...
    df = pd.read_csv('data/HTRU.csv', sep=';')
    df_ar = df.values
    ...
    train, test = train_test_split(df, stratify=df['Decision'], test_size=0.4)
    ...
    for train_index, test_index in skf.split(train, train_y):
        ...
        xopt, fopt = pso(fitness, lb, ub, debug=True, maxiter=20, swarmsize=20)
